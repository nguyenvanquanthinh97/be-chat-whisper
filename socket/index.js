			...
			io
				...
				.on('connect', async (socket) => {
					...
					socket.on('disconnect', async () => {
						try {
							userWithSocket[userId] = get(userWithSocket, userId, []).filter(
							...
							if (userWithSocket[userId].length === 0) {
								...
								io
									.of(`/${companyId}`)
									.emit('userActivityList', { userId, isOnline: false, lastActiviy: moment() });
							}
						} catch (error) {
							socket.emit('error', error);
						}
						...
						try {
							...
							if (fileTypes.includes(messageType)) {
								...
								io
									.of(`/${companyId}`)
									.to(roomId)
									.emit('messageFromServer', clientFormatMessage, roomId, receiveUnread.total);
							}
							...
							if (messageType !== 'text') {
								...
								socket
									.to(roomId)
									.emit('messageFromServer', formatTextMessage, roomId, receiveUnread.total);
								return;
							}
